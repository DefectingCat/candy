//! 静态文件服务模块
//!
//! 提供 HTTP 静态文件服务和目录列表功能。
//!
//! # 功能特性
//!
//! - 静态文件服务：支持各种文件类型，自动检测 MIME 类型
//! - 目录列表：支持自动生成目录列表页面
//! - 默认索引文件：支持配置默认索引文件（如 index.html）
//! - 自定义错误页面：支持配置自定义 404 和其他错误页面
//! - ETag 支持：自动生成 ETag，支持客户端缓存
//! - 范围请求：支持 HTTP Range 请求（部分内容）
//! - 文件流式传输：使用流式传输提高性能
//!
//! # 文件查找逻辑
//!
...
//! 2. 在路由映射中查找对应的根目录
//! 3. 尝试查找请求的文件
//! 4. 如果文件不存在，尝试查找默认索引文件
//! 5. 如果启用了目录列表，生成目录列表页面
//! 6. 如果都失败，返回 404 错误或自定义错误页面
//!
//! # 使用示例
//!
//! ```no_run
//! use candy::http::serve::serve;
//! use axum::Router;
//!
//! // 路由会自动注册静态文件处理器
